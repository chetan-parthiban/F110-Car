...
OccGrid::OccGrid(const nav_msgs::OccupancyGrid &map_msg)
{
    ...
    // if h is 2048, origin_idx is 1023
    // if h is 2049, origin_idx is 1024
    //origin_idx_[0] = static_cast<int>(map_width_/2) + (map_width_%2);
    //origin_idx_[1] = static_cast<int>(map_height_/2) + (map_height_%2);
    origin_metric_[0] = -7.801;
    origin_metric_[1] = -16.388;
    map_resolution_ = map_msg.info.resolution;

    ROS_INFO_STREAM("set resolution");
    ...
    int r = 2*padding_x_+1;
    int c = 2*padding_y_+1;
    ...
    ROS_INFO_STREAM(map_height_);

    ROS_INFO_STREAM("width: ");
    ROS_INFO_STREAM(map_width_);

    ROS_INFO_STREAM("PX: ");
    ROS_INFO_STREAM(padding_x_);

    ROS_INFO_STREAM("PY: ");
    ROS_INFO_STREAM(padding_y_);

    ROS_INFO_STREAM("occ_grid_ size: ");
    ROS_INFO_STREAM(occ_grid_.rows());
    ROS_INFO_STREAM(occ_grid_.cols());
